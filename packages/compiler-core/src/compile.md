...
5. **文本包装成 `TEXT_CALL`**：如果元素子节点里出现了可合并文本，并且当前 `children` 不止一个节点，`transformText` 会把 `TEXT` 或 `COMPOUND_EXPRESSION` 包装成 `TEXT_CALL`，其 `codegenNode` 为 `creareCallExpression(ctx.helper(CREATE_TEXT), args)`，用于表达“这里后面要生成 `createText(...)` 调用”。
6. **动态文本标记**：当被包装的节点不是纯 `TEXT`，而是插值或复合表达式时，会额外给 `createText` 参数补上 `PatchFlags.TEXT`，告诉后续运行时这是一个需要更新的动态文本节点。
7. **回写时机**：`node.children = _children` 放在整轮扫描和包装结束之后，避免遍历过程中一边读旧节点、一边写新节点。
8. **`compile` 返回值**：`transform(ast)` 执行完后直接 `return ast`，便于在浏览器 demo 中观察文本合并、插值改写、`TEXT_CALL` 包装和 helper 收集后的树结构。
