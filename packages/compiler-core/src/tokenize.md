...
- **插值 `{{ … }}`**：`Text` 遇到 `{` 且下一字符为 `{` 时，若前面还有未提交区间则先 `onText`，再以 `sectionStart` 指向 `{{` 处进入 `Interpolation`；`Interpolation` 中遇到 `}}` 时调用 `onInterpolation(sectionStart, index+1)`（区间为整段含双花括号），随后回到 `Text` 并更新 `sectionStart`。插值与标签的衔接顺序与 Vue tokenizer 一致为先提交前置文本再切状态。
...
- **`getPos`**：为 AST `loc` 提供行列与 `offset`；通过记录换行索引，把 `offset` 换算成对应行号和列号（列号为 1-based）。
