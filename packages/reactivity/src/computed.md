...
1. **缓存机制**：计算属性有缓存，只有依赖变化时才重新计算
2. **双重身份**：计算属性既是依赖项也是订阅者
3. **值变化检测**：只有值真正变化时才通知订阅者
4. **只读计算属性**：如果只传入 getter 函数，计算属性是只读的
5. **可写计算属性**：需要传入包含 `get` 和 `set` 的对象
6. **isRef 判断**：计算属性可以通过 `isRef` 判断为 `true`
