...
`dep` 模块负责管理 `reactive` 对象的依赖收集与触发更新。它通过 `WeakMap` + `Map` 建立属性到依赖项的映射。
...
1. **WeakMap 限制**：`targetMap` 使用 `WeakMap`，key 必须是对象，这是 JavaScript 的限制
2. **属性隔离**：每个属性的依赖是独立的，互不影响，实现精确更新
3. **懒加载**：`Dep` 实例和 `depsMap` 都是按需创建的，节省内存，提高性能
4. **依赖检查**：`trigger` 时会检查是否存在依赖，避免不必要的操作，提高效率
5. **与 ref 的区别**：`dep` 用于 `reactive` 对象，`ref` 有自己的依赖管理机制（直接在 RefImpl 中维护）
6. **内存安全**：使用 `WeakMap` 存储，不会阻止对象被垃圾回收，避免内存泄漏
7. **数据结构**：外层使用 `WeakMap`，内层使用 `Map`，兼顾内存安全和性能
