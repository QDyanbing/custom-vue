...
1. **立即执行**：effect 创建后会立即执行一次
2. **自动收集依赖**：副作用函数中访问的响应式数据会自动收集为依赖
3. **嵌套支持**：支持嵌套调用，通过保存和恢复 `activeSub` 处理
4. **停止机制**：停止后不会响应依赖变化，但可以手动执行
5. **调度器**：可以通过 `scheduler` 自定义执行时机
6. **依赖清理**：停止时会自动清理所有依赖关系
