...
`index.ts` 为 `@vue/reactivity` 的统一导出入口。下列清单若与源码不一致，以 `index.ts` 实际 `export` 为准。
...
1. **统一出口**：对外 API 均从 `index.ts` 再导出。
2. **类型**：实现文件中的类型多随值一并导出，可按需 `import type`。
3. **内部模块**：`system.ts`、`dep.ts` 等以支撑实现为主，业务侧一般只从包入口引用。
4. **包名**：文档与示例中的包名为 `@vue/reactivity`。
