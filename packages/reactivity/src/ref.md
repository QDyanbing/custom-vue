...
`ref` 用于创建响应式引用。它可包装任意类型的值（基本类型、对象、数组等）。
...
1. **访问方式**：必须通过 `.value` 访问和修改 ref 的值，这是 ref 的核心特性
2. **对象自动转换**：传入对象会自动转换为 `reactive`，但访问时仍需使用 `.value` 访问整个对象
3. **值变化检测**：只有值真正变化时才会触发更新，使用 `Object.is()` 进行严格相等比较，避免不必要的重新渲染
4. **依赖清理**：当 effect 停止或不再使用时，会自动清理依赖关系，避免内存泄漏
5. **双向链表**：ref 使用双向链表维护订阅者关系，支持高效的依赖收集和更新传播
6. **toRef 和 toRefs**：用于将响应式对象的属性转换为 ref，保持响应性连接
