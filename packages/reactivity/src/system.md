...
`system.ts` 维护 `Dependency`（被订阅方）与 `Sub`（订阅方）之间的双向链表边 `Link`，承担 `link`、`propagate`、追踪区间 `startTrack`/`endTrack` 与 `clearTracking` 回收。行为以源码为准，本文为对照笔记。
...
1. **双向链表**：依赖项和订阅者之间是双向链表关系，删除时需要同时更新两端，保证数据一致性
2. **节点复用**：清理的节点会放入 `linkPool`（对象池），下次需要时复用，减少内存分配和 GC 压力
3. **追踪标志**：`tracking` 和 `dirty` 标志用于避免重复执行和循环依赖，提高性能
4. **尾节点重置**：`startTrack` 时将尾节点重置为 `undefined`，用于判断依赖变化，实现精确的依赖清理
5. **性能优化**：节点复用、避免重复执行、双向链表设计都是性能优化的关键点
6. **依赖清理**：`endTrack` 时会清理不再使用的依赖关系，避免内存泄漏
