...
1. **数据源类型**：支持 ref、reactive 对象和 getter 函数，根据类型自动创建对应的 getter
2. **深度监听**：reactive 对象默认深度监听（`deep: true`），ref 和函数默认不深度监听
3. **清理副作用**：每次回调执行前会清理上一次的副作用，用于清理定时器、取消请求等
4. **停止监听**：返回的 `stop` 函数可以手动停止监听，停止后不再响应数据变化
5. **旧值获取**：第一次执行时，`oldValue` 为 `undefined`（除非 `immediate: true`）
6. **scheduler 机制**：watch 基于 effect 的 scheduler 机制实现，可以控制执行时机并获取新旧值
7. **once 选项**：设置 `once: true` 后，回调执行一次后会自动停止监听
8. **traverse 函数**：深度监听使用 `traverse` 函数递归访问所有属性，触发依赖收集
