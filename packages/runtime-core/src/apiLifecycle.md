...
`apiLifecycle.ts` 提供组件生命周期的注册与触发：用户通过 `onBeforeMount`、`onMounted`、`onBeforeUpdate`、`onUpdated`、`onBeforeUnmount`、`onUnmounted` 注册钩子，渲染器在挂载、更新、卸载的对应时机调用 `triggerHooks` 执行这些钩子。
...
`triggerHooks(instance, type)` 在 `setCurrentInstance(instance)` 包裹下顺序执行 `instance[type]` 中每个钩子，保证钩子内 `getCurrentInstance()` 可用。
...
- **renderer.ts**：在 `setupRenderEffect` 的 componentUpdateFn 中，在挂载/更新分支的合适位置调用 `triggerHooks(instance, LifecycleHooks.xxx)`；在 `unmountComponent` 中调用 `triggerHooks` 触发卸载前后钩子。
